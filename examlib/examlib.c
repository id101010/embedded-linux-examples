...
{    
    ...
    adc_fd = open(AIN4_DEV, O_RDONLY); 					// open adc file descriptor
    ...
    while (1) {
        ...
        charRead = read(adc_fd, adcBuffer, sizeof(adcBuffer));	// get ADC as a string
        
		if (charRead != -1) {

            adcBuffer[charRead] = '\0';			// Stringtermination

            aValue = (float) atoi(adcBuffer);	// Voltage calculation
